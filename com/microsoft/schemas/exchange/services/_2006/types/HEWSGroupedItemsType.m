// Generated by xsd compiler for ios/objective-c
// DO NOT CHANGE!

#import <Foundation/Foundation.h>
#import "HEWSGroupedItemsType.h"
#import "HEWSArrayOfRealItemsType.h"

@implementation HEWSGroupedItemsType

@synthesize groupIndex = _groupIndex;
@synthesize items = _items;

// class meta-data method
// note: this method is only for internal use, DO NOT CHANGE!
+(PicoClassSchema *)getClassMetaData {
    return nil;
}

// property meta-data method
// note: this method is only for internal use, DO NOT CHANGE!
+(NSMutableDictionary *)getPropertyMetaData {
    NSMutableDictionary *map = [NSMutableDictionary dictionary];
    
    PicoPropertySchema *ps = nil;
    ps = [[PicoPropertySchema alloc] initWithKind:PICO_KIND_ELEMENT xmlName:@"GroupIndex" propertyName:@"groupIndex" type:PICO_TYPE_STRING clazz:nil];
    [map setObject:ps forKey:@"groupIndex"];
    [ps release];
    ps = [[PicoPropertySchema alloc] initWithKind:PICO_KIND_ELEMENT xmlName:@"Items" propertyName:@"items" type:PICO_TYPE_OBJECT clazz:[HEWSArrayOfRealItemsType class]];
    [map setObject:ps forKey:@"items"];
    [ps release];
    
    return map;
}



-(void)dealloc {
    self.groupIndex = nil;
    self.items = nil;
    [super dealloc];
}

@end
